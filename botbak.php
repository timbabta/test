...
class ShopBot
{
    ...
    private $admin = "1476035101"; 
    private $helloText = "Выберите товар\nиз каталога 👇";
    private $img_path = "img"; 
    private $geocity = "🌏";

    private $receiver = "";
    private $urlBot = "t.me/bon_username";
    private $nameShop = "Телеграм Магазин";
    ...
    private function router($data)
    {
		...
		$this->setCity($data);
                    ...
                    $this->adminPage();
		 } elseif ($text == "\xF0\x9F\x93\x9D Мои заказы") {
                    $this->userLc($chat_id);
		} elseif ($text == "🛒 Корзина") {
                    $this->showBasket2($data);
	        } elseif ($text == "📖 Каталог") {
                   $this->goStartBot($chat_id,$data);
	        } elseif ($text == "🎀 Акции") {
                   $this->goHelp($chat_id);
                } elseif ($text == "💡 Помошь") {
                   $this->goHelp($chat_id);

		} elseif (stripos($text, $this->geocity) !== false) {
                   $this->setCity($data);
                    ...
                    } elseif (preg_match("~^setprice_1_~", $action) && $this->isAdmin($chat_id)) {
                         $param = explode("_", $action);
                        $this->ePrice($param[2], $param[3], $text);
                    } elseif (preg_match("~^setprice_2_~", $action) && $this->isAdmin($chat_id)) {
                         $param = explode("_", $action);
                        $this->showProduct($param[2], $text);
                    } elseif (preg_match("~^step_1_phone$~", $actionUser)) {
                        $this->savePhoneUser($text, $data);
                    } elseif (preg_match("~^step_2_city$~", $actionUser)) {
                        $this->saveCityUser($text, $data);
		    } elseif (preg_match("~^step_3_adress$~", $actionUser)) {
                        $this->saveAdressUser($text, $data);
		    } elseif (preg_match("~^step_4_comment$~", $actionUser)) {
                        $this->saveCommentUser($text, $data);
		    } else { 
			$this->sendStart($chat_id,$data);
                    }
                }
 ...
 $fields = $this->prepareEdit($param[1]);
            ...
            if ($upMessage['ok']) {
                ...
                $this->notice($data['id'], "Ошибка отображения товарa");
            }
        }
    }
...
private function myCity($data){
  $this->notice($data['id'], "🤣  НЕ НАЖМАЙТЕ НА ГОРОД");
}
  ...
  private function editHide($data)
    {
        // берем необходимую строку с данными
        $obj = $data['data'];
        // разбиваем в массив
        ...
        if ($checkHref->rowCount() === 0) {
            ...
            // готовим запрос
           $cityid=$param[2];
	    $hide_id='hide_id'.$cityid;
            $updateSql = $this->pdo->prepare("UPDATE bot_shop_product SET $hide_id = :hide WHERE id = :id");
            // обновляем видимость
    ...
    private function showCategory($data)
    {
        ...
        $param = explode("_", $obj);
        // запрос на проверку есть ли такая категория в базе
        $checkHref = $this->pdo->prepare("SELECT * FROM bot_shop_category WHERE id = :id ORDER BY id ASC");
        // выполняем запрос
        $checkHref->execute(['id' => $param[1]]);
        // если вернулось ноль строк
        if ($checkHref->rowCount() === 0) {
 ...
 private function prepareEdit($id,$type="")
    {
  ...
  $buttons[][] = $this->buildInlineKeyBoardButton("Назад", "showCategory_" . $item['parent']);

$cityName = $this->pdo->query("SELECT * FROM cityname");
while ($row = $cityName->fetch()) {



$idcity=$row['id'];

$priceid='price_id'.$idcity;
$hideid='hide_id'.$idcity;
$pric=$item[$priceid];
$hid=$item[$hideid];
$hideIcon = $hid ? '🙈' : '🐵';

  $buttons[] = [
$this->buildInlineKeyBoardButton($row['city'], "myCity_".$idcity),
$this->buildInlineKeyBoardButton("$pric", "editPricetext_".$idcity."_".$id),
$this->buildInlineKeyBoardButton($hideIcon, "editHide_" . $id."_".$idcity."_".$hid),
];
}
  ...
  private function priceBridge($data)
    {
$obj = $data['data'];
        // разбиваем в массив
        $param = explode("_", $obj);
        ...
        // если удалось поставить действие админа
        if ($this->setActionAdmin("pricebridge_". $param[2] . "_" . $param[1])) {
            // выводим инструкцию
...
private function editPricetext($data)
    {
  $user_id = $this->admin;
...
if ($this->setActionAdmin("setprice_1_" . $param[2] . "_" . $param[1])) {


   $text = "\nУкажите новую цену товара:";
                    // добавляем кнопку отменить
                    $buttons[] = [
                        $this->buildInlineKeyBoardButton("Отменить", "showProduct_" . $param[2]),
                    ];
                    $fields = [
                        'chat_id' => $this->admin,
                        'text' => $text,
...
private function ePrice($id, $cityid, $text)
    {

            $price='price_id'.$cityid;
            $updateSql = $this->pdo->prepare("UPDATE bot_shop_product SET $price = :price WHERE id = :id");
            $updateSql->execute(['price' => $text, 'id' => $id]);
  $buttons[] = [
                        $this->buildInlineKeyBoardButton("Посмотреть изменение", "showProduct_" . $id),
                    ];
  ...
  $fields = [
     ...
     */
  private function finishPrice($id,$city,$text)
    {
        // если удалось поставить действие админа
            // выводим инструкцию
            $this->sendMessage($this->admin, "Успех");
    }
 ...
 private function sendStart($chat_id,$data)
    {

        // готовим

$text="Выберите действие нажав на кнопку ниже";
$city = $this->pdo->prepare("SELECT city FROM bot_shop_profile WHERE user_id = :user_id");
    $city->execute(['user_id' => $chat_id]);
$town=$city->fetch()['city'];

$buttons_first[0] = [
             $this->buildKeyboardButton("📖 Каталог"),
              $this->buildKeyboardButton("🌏 $town"),
          ];

$buttons_first[1] = [
             $this->buildKeyboardButton("💡 Помошь"),
...
//             $this->buildKeyboardButton("О нас"),
 ...
 $this->sendMessage($chat_id, $text,$buttons_first,true);
        ...
        // выводим
//$buttons[][] = $this->buildInlineKeyBoardButton("START", "gostartBot_0");
 
 }

 private function goStartBot($chat_id,$data)
    {


 $this->startBot($chat_id, $data,"cat");

 $message_id = $this->getMessageId($data);
        $message_id=$message_id + 1;
            $newUser = $this->pdo->prepare("UPDATE bot_shop_profile SET message_id=:message_id WHERE user_id = :user_id");
            $newUser->execute([
                'user_id' => $chat_id,
     ...
     * @param $chat_id
     */
    private function startBot($chat_id, $data,$cat=NULL)
    {
	...
	$message_id = $this->getMessageId($data);
            $newUser = $this->pdo->prepare("INSERT INTO bot_shop_profile SET user_id = :user_id, first_name = :first_name, last_name = :last_name, phone = :phone, adress = :adress, city = :city, action = 'start', message_id=:message_id, comment =:comment ");
            $newUser->execute([
                'user_id' => $chat_id,
...
$this->botApiQuery("deleteMessage", ['chat_id' => $chat_id, 'message_id' => $data['message']['message_id']]);
  ...
  $messag_id = $this->getMessageId($data);


if($cat){
$text = $this->helloText;
$mes = $this->pdo->prepare("SELECT message_id FROM bot_shop_profile WHERE user_id = :chat_id");
  $mes->execute(['chat_id' => $chat_id]);
  $mess=$mes->fetch()['message_id'];


 $this->botApiQuery("deleteMessage", ['chat_id' => $chat_id, 'message_id' => $mess]);
...
// проверяем пользователя на админа
        ...
        $category = $this->pdo->query('SELECT * FROM bot_shop_category WHERE hide = 0 ORDER BY id ASC');
...
$buttons[][] = $this->buildInlineKeyBoardButton("Актобе","selectCity_1" );
$buttons[][] = $this->buildInlineKeyBoardButton("Алматы","selectCity_2" );
$buttons[][] = $this->buildInlineKeyBoardButton("Караганда","selectCity_3" );
$buttons[][] = $this->buildInlineKeyBoardButton("Костанай","selectCity_4" );
$buttons[][] = $this->buildInlineKeyBoardButton("Нур-Султан","selectCity_5" );
$buttons[][] = $this->buildInlineKeyBoardButton("Уральск","selectCity_6" );
...
private function selectCity($data)
{


$user_id = $this->getChatId($data);
$this->botApiQuery("deleteMessage", ['chat_id' => $user_id, 'message_id' => $data['message']['message_id']]);

$param = explode("_", $data['data']);
$text=$param[1];
	if($this->setParamUser('city_id', $text, $user_id)){
		if($text=='1'){
		$this->setParamUser('city', 'Актобе', $user_id);
		}
		elseif($text=='2'){
	        $this->setParamUser('city', 'Алматы', $user_id);
		}
                elseif($text=='3'){
                $this->setParamUser('city', 'Караганда', $user_id);
                }
                elseif($text=='4'){
                $this->setParamUser('city', 'Костанай', $user_id);
                }
                elseif($text=='5'){
                $this->setParamUser('city', 'Нур-Султан', $user_id);
                }
                elseif($text=='6'){
                $this->setParamUser('city', 'Уральск', $user_id);
                }
$city = $this->pdo->prepare("SELECT city FROM bot_shop_profile WHERE user_id = :user_id");
    $city->execute(['user_id' => $user_id]);
$town=$city->fetch()['city'];

$text_="Выбран регион 🌏 ".$town;
...
$buttons_first[1] = [
 ...
 $this->sendMessage($user_id,$text_,$buttons_first,true);

 	$this->goStartBot($user_id,$data);
 	}
}

private function goHelp($chat_id)
{

$text="Часто задаваемые вопросы";
 $buttons[][] = $this->buildInlineKeyBoardButton("Как Оплатить","howBy_0" );
// $buttons[][] = $this->buildInlineKeyBoardButton("Как проверить статус заказа","setCity_0" );


$this->sendMessage($chat_id, $text, $buttons);
}

private function howOr($data)
{

$user_id = $this->getChatId($data);
$text = "<b>Мы пока работаем над этим</b>\n";
...
private function howBy($data)
{

$user_id = $this->getChatId($data);
$text = "<b>Как Опатить заказ</b>\n";
$text.="\n💵При наличном рассчете просим оплатить заказ после получения и осмотра товара\n";
//$text.="\n💳При безналичном рассчете,необходимо выполнить следующие дйствие\n";
$text.="\n💳При безналичном рассчете,необходимо выполнить следующие действие\n";
$text.="\n\n1️⃣ Полностью оформите заказ при помощи чат бота";
$text.="\n2️⃣ После подтверждения заказа, отобразится информация о сумме к оплате";
$text.="\n3️⃣ Далее необходимо пройти в приложение Каспи Банка, \nвыбрать категорию <b>Платежи➡Покупки и заказы➡Вода➡ Вода Хрустальная</b>";
$text.="\n4️⃣ Введите сумму которая ранее отобразилась при оформлении заказа, и проведите оплату";
...
$this->sendMessage($user_id, $text);
    ...
    private function showBasket2($data)
    {
        // получаем данные
        $chat_id = $this->getChatId($data);
	 $message_id = $this->getMessageId($data);
    ...
    private function showBasket($data)
    {
      ...
      $message_id = $this->getMessageId($data);
        ...
        @$this->setActionUser("show_basket", $chat_id);
        // Выводим корзину
        $this->showBasketBegin($chat_id, $data,$message_id);
        // глушим уведомление
        $this->notice($data['id']);
    }
     ...
     * @param $data
     */
    private function showBasketBegin($user_id, $data,$message_id=0)
    {
  ...
  $mes = $this->pdo->prepare("SELECT message_id FROM bot_shop_profile WHERE user_id = :user_id");
  $mes->execute(['user_id' => $user_id]);
  $mess=$mes->fetch()['message_id'];
	$message_id = $this->getMessageId($data);


 $this->botApiQuery("deleteMessage", ['chat_id' => $user_id, 'message_id' => $message_id]);
 $this->botApiQuery("deleteMessage", ['chat_id' => $user_id, 'message_id' => $mess]);
...
usleep(5000);
    ...
    private function drawBasketButton($begin, $basketCount, $item, $sum)
    {
        ...
        $buttons[] = [
            ...
            $this->buildInlineKeyBoardButton($item['count'].' шт.', 'basketViewParam_' . $item['count'] . '_0'),
    ...
    private function savePhoneUser($text, $data)
    {
        ...
        if (preg_match("/^\+[0-9]{9,14}$/i", $text)) {
            if ($this->setActionUser("step_3_adress", $user_id)) {
   $message_id = $this->getMessageId($data);
    $mess=$message_id-1;

  $newmes = $this->pdo->prepare("UPDATE bot_shop_profile SET message_id=:message_id WHERE user_id = :user_id");
  $newmes->execute([ 'message_id' => $mess,'user_id' => $user_id]);
...
private function saveCityUser($text, $data) {
    $user_id = $this->getChatId($data);
    // Достаем телефон
...
$message_id = $this->getMessageId($data);
  ...
  $newmes->execute([ 'message_id' => $mess,'user_id' => $user_id]);
        ...
        if ($this->setParamUser('city', $text, $user_id)) {
...
private function saveAdressUser($text, $data) {
 ...
 $city = $this->pdo->prepare("SELECT city FROM bot_shop_profile WHERE user_id = :user_id");
    ...
    // !!!!!!!!!!!!!!!!!!
    // устанавливаем пользователю следующий шаг ДЛЯ НОВОГО ПОЛЯ 1
    if ($this->setActionUser("step_4_comment", $user_id)) {
$message_id = $this->getMessageId($data);
    $mess=$message_id-1;
$this->botApiQuery("deleteMessage", ['chat_id' => $user_id, 'message_id' => $message_id]);
  ...
  $newmes->execute([ 'message_id' => $mess,'user_id' => $user_id]);
        ...
        if ($this->setParamUser('adress', $text, $user_id)) {
	     ...
	     $text_ .= "\nАдрес доставки: " . $text;
            ...
            $text_ .= "\n\nУкажите комментарий желаемое время и дату доставки: ";
            // $buttons[][] = $this->buildInlineKeyBoardButton('✔ Готово', 'setReady_0');
    ...
    private function saveCommentUser($text, $data)
    {
        $user_id = $this->getChatId($data);
        // Достаем телефон
        $phone = $this->pdo->prepare("SELECT phone FROM bot_shop_profile WHERE user_id = :user_id");
...
$this->botApiQuery("deleteMessage", ['chat_id' => $user_id, 'message_id' => $mess]);
  ...
  $newmes->execute([ 'message_id' => $mess,'user_id' => $user_id]);
            ...
            if ($this->setParamUser('comment', $text, $user_id)) {
                $text_ = "<b>Оформление заказа</b>\n\n";
                // сумма заказа
                $text_ .= "Сумма заказа: " . $this->totalSumOrder($user_id) . " тенге";
                // телефон
                $text_ .= "\nТелефон: " . $phone->fetch()['phone'];
	        $text_ .= "\nГород: " . $city->fetch()['city'];
	        $text_ .= "\nАдрес: " . $adress->fetch()['adress'];
                $text_ .= "\nКомментарий: " . $text;
	        $text_ .= "\nНажмите на кнопку <b>Готово</b> ";
               $buttons[][] = $this->buildInlineKeyBoardButton('✔ Готово', 'setReady_0');
            } else {
                $text_ = "Ошибка попробуйте снова /start";
            }
    ...
    private function setReady($data)
    {
        ...
        if ($basket->rowCount() > 0) {
            ...
            $inOrder = $this->pdo->prepare("INSERT INTO bot_shop_order SET user_id = :user_id, date = NOW(), status = 0, name = :name, phone = :phone, adress = :adress, city = :city, comment = :comment");
            if ($inOrder->execute([
                'user_id' => $user_id,
		...
		'comment' => $userInfo['comment'],
                ...
                $this->notice($data['id'],"Заказ принят в обработку \xF0\x9F\x98\x8A");
		$text="Заказ принят в обработку \xF0\x9F\x98\x8A.\nВ случае необходимости,\nменеджер контакт центра свяжется с вами \xE2\x98\x8E";
		$message_id=$data['message']['message_id'];
	       $this->editMessageText($user_id, $message_id);

$text="Посмтреть историю и статус заказов вы сможете нажав на кнопку \n\xF0\x9F\x93\x9D Мои заказы";
$city = $this->pdo->prepare("SELECT city FROM bot_shop_profile WHERE user_id = :user_id");
    $city->execute(['user_id' => $user_id]);
$town=$city->fetch()['city'];
...
$buttons_first[1] = [
 ...
 $this->sendMessage($user_id,$text,$buttons_first,true);
    ...
    private function drawOrder($user_id = 0, $begin = 0)
    {
	    ...
	    $text .= "<b>Номер заказа: №" . $orderRaw['id'] . "</b>\n";
...
$status="Выполнен 👍";
	    ...
	    $text .= "<b>Город:</b> " . $orderRaw['city'] . "\n";
            $text .= "<b>Адрес:</b> " . $orderRaw['adress'] . "\n";
	    $text .= "<b>Комментарий:</b> " . $orderRaw['comment'] . "\n";
            ...
            if (!$orderRaw['status']) {
                if (!$orderRaw['type_pay'] && $user_id) {
                    ...
                    $buttons[][] = $this->buildInlineKeyBoardButton('Оплатить через Kaspi', '', $url);
            ...
            if ($count > 1) {
                ...
                // выводим навигацию
                $buttons[] = [
                    $this->buildInlineKeyBoardButton("\xE2\x8F\xAA", 'orderGo_' . $prev . '_' . $user_id),
                    $this->buildInlineKeyBoardButton("\xE2\x8F\xA9", 'orderGo_' . $next . '_' . $user_id),
                ];
            }
        } else {
...
*/
 ...
 if (!is_null($buttons) && is_array($buttons)) {
     ...
     * @return mixed
     */
    private function editMessageText($user_id, $message_id, $text, $buttons = NULL,$type = false)
    {
        // готовим массив данных
        $data_send = [
            'chat_id' => $user_id,
