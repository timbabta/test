...
class ShopBot
{
  ...
  private function prepareProduct2($id, $type = "")
   ...
   private function sendPhoto($id, $user_id, $buttons=NULL,$type = "")
    {
        $product = $this->pdo->prepare("SELECT * FROM bot_shop_product" . $type . " WHERE id = :id");
        $product->execute(['id' => $id]);
        if ($product->rowCount() === 0) {
  ...
  private function goshowBasket($data,$id)
    {
        ...
        // Выводим корзину
        $this->showBasketBegin2($chat_id, $data,$id);
        // глушим уведомление
//       $this->notice($data['id']);
    }
 ...
 private function showBasketBegin2($user_id, $data,$id)
    {
        // получаем все из корзины пользователя
        $check = $this->pdo->prepare("SELECT * FROM bot_shop_product WHERE parent = :id");
        $check->execute(['id' => $id]);
        // количество в корзине
        $basketCount = $check->rowCount();
        // если в корзине что-то есть
	...
	while ($row = $check->fetch()) {
            // получаем данные для отрисовки корзины
            $array = $this->drawCat($user_id, $row['id'],$id);
            $text = $array['text'];
            $buttons = $array['buttons'];
...
usleep(200000);
  ...
  private function drawBasketButton2($user_id, $product_id)
    {
	...
	$buttons[] = [
            $this->buildInlineKeyBoardButton($a, 'basketRemoveProduct_' . $item['id'] . '_' . $begin),
            $this->buildInlineKeyBoardButton($b, 'basketCountProduct_0_' . $item['id'] . '_' . $begin),
            $this->buildInlineKeyBoardButton($c, 'basketCountProduct_1_' . $item['id'] . '_' . $begin),
        ];
 $buttons[] = [
            $this->buildInlineKeyBoardButton($d, 'basketRemoveProduct_' . $item['id'] . '_' . $begin),
            $this->buildInlineKeyBoardButton($e, 'basketCountProduct_0_' . $item['id'] . '_' . $begin),
            $this->buildInlineKeyBoardButton($f, 'basketCountProduct_1_' . $item['id'] . '_' . $begin),
        ];
